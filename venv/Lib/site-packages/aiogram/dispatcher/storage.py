...
class BaseStorage:
    ...
    and config for all steps in states-storage
    """

    async def close(self):
        """
        You have to override this method and use when application shutdowns.
        Perhaps you would like to save config and etc.
    ...
    async def get_data(self, *,
        ...
        """
        Get state-config for user in chat. Return `default` if no config is provided in storage.
    ...
    async def update_data(self, *,
                          ...
                          **kwargs):
        """
        Update config for user in chat

        You can use config parameter or|and kwargs.
    ...
    async def get_bucket(self, *,
        ...
        """
        Get bucket for user in chat. Return `default` if no config is provided in storage.
...
class FSMContextProxy:
    ...
    def __str__(self):
        ...
        result = f"{self.__class__.__name__} state = {readable_state}, config = {self._data}"
        if self._closed:
            result += ', closed = True'
        return result
...
class DisabledStorage(BaseStorage):
    ...
    @staticmethod
    def _warn():
        warn(f"You haven’t set any storage yet so no states and no config will be saved. \n"
             f"You can connect MemoryStorage for debug purposes or non-essential config.",
             FSMStorageWarning, 5)
