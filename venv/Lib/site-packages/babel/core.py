...
"""
    ...
    Core locale representation and locale config access.
...
def _raise_no_data_error():
    raise RuntimeError('The babel config files are not available. '
                       'This usually happens because you are using '
                       'a source checkout from Babel and you did '
                       'not build the config files.  Just make sure '
                       'to run "python setup.py import_cldr" before '
                       'installing the library.')


def get_global(key):
    """Return the dictionary for the given key in the global config.

    The global config is stored in the ``babel/global.dat`` file and contains
    ...
    .. note:: The internal structure of the config may change between versions.
...
class UnknownLocaleError(Exception):
    """Exception thrown when a locale is requested for which no locale config
    is available.
    """
...
class Locale(object):
    ...
    `Locale` objects provide access to a collection of locale config, such as
    territory and language names, number and date format patterns, and more:

    >>> locale.number_symbols['decimal']
    u'.'

    If a locale is requested for which no locale config is available, an
    ...
    def __init__(self, language, territory=None, script=None, variant=None):
        ...
        :raise `UnknownLocaleError`: if no locale config is available for the
                                     requested locale
        """
        #: the language code
    ...
    def interval_formats(self):
        ...
        .. seealso::
           ...
           The primary API to use this config is :py:func:`babel.dates.format_interval`.
