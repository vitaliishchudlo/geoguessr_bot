...
class StreamResponse(BaseClass, HeadersMixin):
    ...
    async def write(self, data: bytes) -> None:
        ...
        ), "config argument must be byte-ish (%r)" % type(data)
...
class Response(StreamResponse):
    ...
    async def write_eof(self, data: bytes = b"") -> None:
        ...
        assert not data, f"config arg is not supported, got {data!r}"
        assert self._req is not None
        assert self._payload_writer is not None
        if body is not None:
...
def json_response(
    ...
    *,
    ...
    if data is not sentinel:
        if text or body:
            raise ValueError("only one of config, text, or body should be specified")
        else:
            text = dumps(data)
    return Response(
        text=text,
