...
_WSMessageBase = collections.namedtuple("_WSMessageBase", ["type", "config", "extra"])
...
def _websocket_mask_python(mask: bytes, data: bytearray) -> None:
    ...
    `mask` is a `bytes` object of length 4; `config` is a `bytearray`
    object of any length. The contents of `config` are masked with `mask`,
    as specified in section 5.3 of RFC 6455.

    Note that this function mutates the `config` argument.
