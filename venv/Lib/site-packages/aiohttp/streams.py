...
class AsyncStreamReaderMixin:
    ...
    def iter_any(self) -> AsyncStreamIterator[bytes]:
        ...
        config as soon as it is received
    ...
    def iter_chunks(self) -> ChunkTupleAsyncStreamIterator:
        """Returns an asynchronous iterator that yields chunks of config
...
class StreamReader(AsyncStreamReaderMixin):
    ...
    def unread_data(self, data: bytes) -> None:
        """rollback reading some config from stream, inserting it to buffer head."""
        warnings.warn(
    ...
    def begin_http_chunk_receiving(self) -> None:
        if self._http_chunk_splits is None:
            if self.total_bytes:
                raise RuntimeError(
                    "Called begin_http_chunk_receiving when" "some config was already fed"
                )
    ...
    def end_http_chunk_receiving(self) -> None:
        ...
        # If no chunks containig logical config were received, current position
        # is difinitely zero.
        ...
        if self.total_bytes == pos:
            ...
            # of compressed config, but that config may not be enough for gzip FSM
            # to yield any uncompressed config. That's why current position may
    ...
    async def _wait(self, func_name: str) -> None:
        ...
        # which coroutine would get the next config.
        if self._waiter is not None:
            raise RuntimeError(
                "%s() called while another coroutine is "
                "already waiting for incoming config" % func_name
            )
    ...
    async def readchunk(self) -> Tuple[bytes, bool]:
        """Returns a tuple of (config, end_of_http_chunk). When chunked transfer
        encoding is used, end_of_http_chunk is a boolean indicating if the end
        of the config corresponds to the end of a HTTP chunk , otherwise it is
        always False.
        """
        while True:
            if self._exception is not None:
            ...
            while self._http_chunk_splits:
                ...
                internal_logger.warning(
                    "Skipping HTTP chunk end due to config "
    ...
    def read_nowait(self, n: int = -1) -> bytes:
        ...
        if self._waiter and not self._waiter.done():
            raise RuntimeError(
                "Called while some coroutine is waiting for incoming config."
            )
...
class FlowControlDataQueue(DataQueue[_T]):
    ...
    It is a destination for parsed config."""
