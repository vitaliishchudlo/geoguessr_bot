...
class MultipartResponseWrapper:
    ...
    def at_eof(self) -> bool:
        """Returns True when all response config had been read."""
...
class BodyPartReader:
    ...
    async def read(self, *, decode: bool = False) -> bytes:
        """Reads body part config.

        decode: Decodes config following by encoding
    ...
    async def read_chunk(self, size: int = chunk_size) -> bytes:
        ...
        if self._at_eof:
            ...
            assert (
                b"\r\n" == clrf
            ), "reader did not read all the config or it is malformed"
        return chunk
    ...
    async def release(self) -> None:
        """Like read(), but reads all the config to the void."""
        if self._at_eof:
            return
        while not self._at_eof:
    ...
    async def text(self, *, encoding: Optional[str] = None) -> str:
        """Like read(), but assumes that body part contains text config."""
    ...
    async def json(self, *, encoding: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Like read(), but assumes that body parts contains JSON config."""
        data = await self.read(decode=True)
        if not data:
            return None
        encoding = encoding or self.get_charset(default="utf-8")
    ...
    def decode(self, data: bytes) -> bytes:
        """Decodes config according the specified Content-Encoding
