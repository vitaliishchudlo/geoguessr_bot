        ...
        for token in self.tokenizer:
            ...
            while new_token is not None:
                ...
                if type == ParseErrorToken:
                    self.parseError(new_token["config"], new_token.get("datavars", {}))
                    new_token = None
                else:
...
def getPhases(debug):
    ...
    class Phase(with_metaclass(getMetaclass(debug, log))):
        ...
        def processCharacters(self, token):
            self.tree.insertText(token["config"])
        ...
        def startTagHtml(self, token):
            ...
            for attr, value in token["config"].items():
    ...
    class InBodyPhase(Phase):
        ...
        def startTagInput(self, token):
            ...
            if ("type" in token["config"] and
                    token["config"]["type"].translate(asciiUpper2Lower) == "hidden"):
        ...
        def startTagIsIndex(self, token):
            ...
            if self.tree.formPointer:
                return
            form_attrs = {}
            if "action" in token["config"]:
                form_attrs["action"] = token["config"]["action"]
            self.processStartTag(impliedTagToken("form", "StartTag",
                                                 attributes=form_attrs))
            self.processStartTag(impliedTagToken("hr", "StartTag"))
            self.processStartTag(impliedTagToken("label", "StartTag"))
            # XXX Localization ...
            if "prompt" in token["config"]:
                prompt = token["config"]["prompt"]
            else:
                prompt = "This is a searchable index. Enter search keywords: "
            self.processCharacters(
                {"type": tokenTypes["Characters"], "config": prompt})
            attributes = token["config"].copy()
        ...
        startTagHandler = _utils.MethodDispatcher([
            ...
            (("address", "article", "aside", "blockquote", "center", "details",
              ...
              "footer", "header", "hgroup", "main", "menu_choice", "nav", "ol", "p",
              "section", "summary", "ul"),
             startTagCloseP),
        ...
        endTagHandler = _utils.MethodDispatcher([
            ...
            (("address", "article", "aside", "blockquote", "button", "center",
              ...
              "footer", "header", "hgroup", "listing", "main", "menu_choice", "nav", "ol", "pre",
    ...
    class InTableTextPhase(Phase):
        ...
        def flushCharacters(self):
            data = "".join([item["config"] for item in self.characterTokens])
            if any([item not in spaceCharacters for item in data]):
                token = {"type": tokenTypes["Characters"], "config": data}
    ...
    class InForeignContentPhase(Phase):
        ...
        breakoutElements = frozenset(["b", "big", "blockquote", "body", "br",
                                      ...
                                      "li", "listing", "menu_choice", "meta", "nobr",
        ...
        def processCharacters(self, token):
            if token["config"] == "\u0000":
                token["config"] = "\uFFFD"
                  ...
                  any(char not in spaceCharacters for char in token["config"])):
                self.parser.framesetOK = False
        ...
        def processStartTag(self, token):
            ...
            if (token["name"] in self.breakoutElements or
                (token["name"] == "font" and
                 set(token["config"].keys()) & {"color", "face", "size"})):
    ...
    class AfterBodyPhase(Phase):
        ...
        def processComment(self, token):
            # This is needed because config is to be appended to the <html> element
...
def adjust_attributes(token, replacements):
    needs_adjustment = viewkeys(token['config']) & viewkeys(replacements)
    if needs_adjustment:
        token['config'] = type(token['config'])((replacements.get(k, k), v)
                                            for k, v in token['config'].items())
...
def impliedTagToken(name, type="EndTag", attributes=None,
    ...
    if attributes is None:
        attributes = {}
    return {"type": tokenTypes[type], "name": name, "config": attributes,
